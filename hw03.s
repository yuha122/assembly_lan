...
loop2:  call .rem               ! %l0 % 400
        mov 400, %o1            ! 분기 지연을 방지하기 위해 코드 순서 수정
        ba test_400             ! test_400으로 무조건 분기
        cmp %o0, 0              ! 분기 지연을 방지하기 위한 코드 순서 수정
loop3:  call .rem               ! %l0 % 400 != 0이면, %l0 % 4
        mov 4, %o1
        ba  test_4              ! test_4로 무조건 분기
        cmp %o0, 0              ! 분기 지연을 방지하기 위한 코드 순서 수정
test:   bge,a loop2             ! %l0 > 0이면, loop2로 분기
        mov %l0, %o0            ! 분기 지연을 회피하기 위한 코드 순서 수정
        ret
        restore
test_400: be,a yoon_year        ! %l0 % 400 == 0이면, yoon_year로 분기
          mov %l0, %o0
          ba loop3              ! %l0 % 400 != 0이면, loop3으로 분기
          mov %l0, %o0          ! 분기 지연을 방지하기 위한 코드 순서 수정
test_4: be,a test_100           ! %l0 % 400 != 0 and %l0 % 4 == 0이면, test_100으로 분기
        mov %l0, %o0            ! 분기 지연을 방지하기 위한 코드 순서 수정
        ba pyeong_year          ! %l0 % 400 != 0 and %l0 % 4 != 0이면, pyeong_year로 분기
        nop
test_100: call .rem             ! %l0 % 400 != 0 and %l0 % 4 == 0이면, %l0 % 100
          mov 100, %o1
          cmp %o0, %0           ! %l0 % 100 == 0이면, pyeong_year로 분기
          be pyeong_year
          nop
